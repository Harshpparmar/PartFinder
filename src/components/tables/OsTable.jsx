...
const OsTable = ({ osData, onSelectOs }) => {
  return (
    <>
            ...
            <table className='w-full table-auto text-sm sm:text-base'>
              <thead className='hidden bg-gray-200 text-gray-700 lg:table-header-group'>
                <tr>
                  ...
                  <th className='px-2 py-2 text-left font-medium sm:px-4 sm:py-3'>Mode</th>
                ...
                {osData.map((os, index) => (
                  <tr key={index} className='block border-b lg:table-row'>
                    <td className='block cursor-pointer px-2 py-2 font-medium hover:underline sm:px-4 sm:py-3 lg:table-cell'>
                      <img src={os.imageUrl} alt={os.name} className='h-8 w-8 inline-block mr-2' />
                      <span className='font-semibold lg:hidden'>Operating System:</span>
                      {os.name}
                    </td>
                    <td className='block px-2 py-2 sm:px-4 sm:py-3 lg:table-cell'>
                      <span className='font-semibold lg:hidden'>Mode:</span>
                      {os.mode}
                    </td>
                    <td className='block px-2 py-2 sm:px-4 sm:py-3 lg:table-cell'>
                    ...
                    <td className="block px-2 py-2 text-right sm:px-4 sm:py-3 lg:table-cell">
                      ...
                      <a href={os.buyLink} className="rounded bg-blue-500 px-3 py-1 text-xs font-medium text-white hover:bg-blue-600 sm:text-sm" target='_blank'>Buy</a>
                    </td>
                    <td className="block px-2 py-2 text-right sm:px-4 sm:py-3 lg:table-cell">

                    <button onClick={() => onSelectOs(os)} className="rounded bg-green-500 px-3 py-1 text-xs font-medium text-white hover:bg-green-600 sm:text-sm">Select</button>
                    </td>
                  </tr>
                ))}
              </tbody>
